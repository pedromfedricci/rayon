...
fn index_max_by<'a, T, F>(v: &'a [T], is_less: F) -> Option<usize>
where
    F: Fn(&T, &T) -> bool + Sync,
    [T]: crate::iter::IntoParallelRefIterator<'a, Item = &'a T>,
    <[T] as crate::iter::IntoParallelRefIterator<'a>>::Iter: crate::iter::IndexedParallelIterator,
{
    use crate::iter::{IndexedParallelIterator, IntoParallelRefIterator, ParallelIterator};
...
fn index_min_by<'a, T, F>(v: &'a [T], is_less: F) -> Option<usize>
where
    F: Fn(&T, &T) -> bool + Sync,
    [T]: crate::iter::IntoParallelRefIterator<'a, Item = &'a T>,
    <[T] as crate::iter::IntoParallelRefIterator<'a>>::Iter: crate::iter::IndexedParallelIterator,
...
pub(super) fn par_partition_at_index<T, F>(
    v: &mut [T],
    index: usize,
    mut is_less: F,
) -> (&mut [T], &mut T, &mut [T])
    ...
    F: Fn(&T, &T) -> bool + Sync,
{
    ...
    if mem::size_of::<T>() == 0 {
        ...
        // We're free to use `unwrap()` here because we know v must not be empty.
        let max_index = index_max_by(v, &is_less).unwrap();
        v.swap(max_index, index);
    } else if index == 0 {
        // Find min element and place it in the first position of the array.
        // We're free to use `unwrap()` here because we know v must not be empty.
        let min_index = index_min_by(v, &is_less).unwrap();
